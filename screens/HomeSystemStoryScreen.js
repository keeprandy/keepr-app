...
// Keeping these around in case we want them later,
// but they’re no longer used directly in this screen.
const escHtml = (s) =>
  String(s ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
...
function getStandardMeta(systemRow) {
  ...
  // legacy fallback (older screens stored identity/warranty at top level)
  return {
...
export default function HomeSystemStoryScreen({ route, navigation }) {
  ...
  // Web-first: Redfin-style two-column header (media left, details right)
  ...
  const {
    items: systemPlacementRows,
    loading: attachmentsLoading,
  } = useAttachments("system", systemId);
  ...
  // Keepr Pro associations (selected in EditSystemEnrichment)
...
const markRecordThumbFailed = useCallback((url) => {
  if (!url) return;
  setRecordThumbErrorByUrl((prev) =>
    prev[url] ? prev : { ...prev, [url]: true }
  );
}, []);


  // ---- data load: system + service records (with service_record_photos) ----
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
        ...
        const photos = rawPhotos
          .map((p) => {
            const url =
              p.url ||
              (p.storage_path
                ? // in production this is usually a signed or public URL;
                  // for now, treat storage_path as unresolved and just keep url if present
                  null
                : null);
      ...
      console.error("HomeSystemStoryScreen loadAll error:", e);
  ...
  // Standard system metadata (preferred) + legacy fallback.
  const {
  ...
  // Load assigned Keepr Pros (if any) for quick action from the story screen.
  useEffect(() => {
    let cancelled = false;
    ...
    const run = async () => {
      try {
        ...
        // Keep order stable: match saved id order when possible, then name.
        ...
        console.log("HomeSystemStory: load keepr pros error", e?.message || e);
        if (!cancelled) {
          setAssignedPros([]);
          setProsLoading(false);
...
// Refresh when navigating back from EditSystemEnrichment (or any child screen).
// Prevents “changes don’t show until manual refresh”.
useFocusEffect(
  useCallback(() => {
    loadAll();
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      for (const row of systemPlacementRows) {
        ...
        if (!previewUrl && row.storage_path) {
          try {
            ...
            console.log(
              "HomeSystemStory: getSignedUrl error",
              e?.message || e
            );
          }
        }
      ...
      if (!cancelled) {
        ...
        // reset only for current list keys (prevents permanent failure state after refresh)
  ...
  const viewerCollection = useMemo(
    () =>
      attachmentPreview.map((a) => ({
        ...a,
        urls: {
          signed: a.previewUrl || a.url || null,
        },
  ...
  const goToSystems = () => {
    // For now, “Systems” just returns to the previous systems list.
    handleBack();
  };
...
// ---- hero selection ----
// Store selected hero on the system record (systems.hero_attachment_id).
// This is durable (cross-device) and avoids reworking the attachments engine.
const setHeroAttachment = useCallback(
  async (att) => {
    ...
    try {
      ...
      // Update local state immediately so the hero swaps without a manual refresh.
      setSystem((prev) =>
        prev ? { ...prev, hero_attachment_id: heroId } : prev
      );
    } catch (e) {
      console.error("setHeroAttachment error:", e);
  ...
  const openViewerAt = useCallback(
    (idx) => {
      ...
      // Web: PDFs are more reliable opened in a new tab (AttachmentViewerModal is mobile-first).
      if (IS_WEB && att) {
        const url = att.url || att?.urls?.signed || null;
        const fileName =
          att.fileName || att.file_name || att.name || att.title || "";
        const isPdf =
          att.isPdf ||
...
const warrantyStarts = warrantyMeta?.starts_on || warrantyMeta?.start_on || warrantyMeta?.start_date || null;
    ...
    return (
      <SafeAreaView style={layoutStyles.screen}>
        <View style={styles.fallbackContainer}>
          ...
          <TouchableOpacity
            style={[styles.chip, { marginTop: spacing.md }]}
            onPress={handleBack}
          >
            <Ionicons
  ...
  const currentAttachment =
    viewerCollection.length &&
    viewerIndex >= 0 &&
    viewerIndex < viewerCollection.length
      ...
      <View style={styles.container}>
        ...
        <View style={styles.headerChipsRow}>
          <TouchableOpacity style={[styles.chip, styles.chipPrimary]}>
            ...
            />
            <Text
              style={[styles.chipLabel, styles.chipLabelPrimary]}
            >
              Story & timeline
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            style={styles.chip}
            onPress={handlePrintSystemStory}
          >
            <Ionicons
              name="print-outline"
              size={14}
        ...
        >
          {/* HERO (attachments driven) */}
<View style={[styles.heroCard, isWide && styles.heroCardWide]}>
  {/* Left: media (click to open viewer when present) */}
  <TouchableOpacity
  ...
  : "Add attachments (photos, files, links) for this system to see them here."}
    ...
    <View style={styles.heroMetaRow}>
      ...
      <Text style={styles.heroMetaText}>{lastServiceLabel}</Text>
          ...
          {/* Right: system details (wide only) */}
  {isWide ? (
    
    <View style={styles.heroDetails}>
            <View style={styles.detailCard}>
        <Text style={styles.detailTitle}>Keepr Pro Assigned</Text>
      ...
      <View style={styles.detailCard}>
        ...
        <View style={styles.detailActionsRow}>
          ...
          {/* Mobile: system details + warranty + Keepr Pro */}
          {!isWide ? (
              ...
              <View style={styles.detailCard}>
                ...
                <View style={styles.kvGrid}>
                  <View style={styles.kvRow}>
                    <Text style={styles.kvLabel}>Manufacturer</Text>
                    <Text style={styles.kvValue} numberOfLines={1}>
                      {identityMeta?.manufacturer || identityMeta?.brand || "—"}
                    </Text>
                  </View>
                  <View style={styles.kvRow}>
                    <Text style={styles.kvLabel}>Model</Text>
                  ...
                  <View style={styles.kvRow}>
                    <Text style={styles.kvLabel}>Serial</Text>
                    <Text style={styles.kvValue} numberOfLines={1}>
                      {identityMeta?.serial_number || identityMeta?.serial || "—"}
                    </Text>
                  </View>
                  <View style={styles.kvRow}>
                    <Text style={styles.kvLabel}>Installed</Text>
                    <Text style={styles.kvValue} numberOfLines={1}>
                      {fmtDate(identityMeta?.installed_on || identityMeta?.installedOn) || "—"}
                    </Text>
                  </View>
                  <View style={styles.kvRow}>
                    <Text style={styles.kvLabel}>Location</Text>
          ...
          <View style={styles.sectionCard}>
            ...
            <Text style={styles.sectionSubtitle}>
              {photos} photo{photos === 1 ? "" : "s"} · {files} file
              {files === 1 ? "" : "s"} · {links} link
              {links === 1 ? "" : "s"}
            </Text>
            ...
            {!hasAnyAttachments ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>
                  No attachments linked to this system yet. Use
                  “Manage attachments” to add proof, manuals, and
                  context.
                </Text>
              </View>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <View style={styles.attachmentThumb}>
                        <Pressable
                          style={[
                            styles.heroSetButton,
                            isHero && styles.heroSetButtonActive,
                          ]}
                          onPress={(e) => {
                            if (IS_WEB && e && typeof e.stopPropagation === "function") {
                              e.stopPropagation();
                            }
                        ...
                        >
                          ...
                          />
                          {!isHero && (
                            <Text style={styles.heroSetButtonLabel}>Hero</Text>
                          )}
                        </Pressable>
