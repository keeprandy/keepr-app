-- Org members should enter the organization workspace first.
-- Personal Keepr remains available as a workspace, but it should not trigger
-- consumer onboarding before an OEM/Dealer/Pro user can reach their org shell.
...
begin
  ...
  return jsonb_build_object(
    'active_workspace_id', coalesce(v_active_workspace_id, v_org_workspaces -> 0 ->> 'workspace_id', v_personal ->> 'workspace_id'),
