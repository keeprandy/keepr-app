-- Expose exact-build metadata from canonical assets in the normal fleet payload.
--
-- This keeps Fleet generic: KF018 appears because it is an asset with an OEM
-- relationship, and the exact-build route is resolved from asset metadata.
...
begin
  ...
  with relationship_rows as (
    select
      ...
      a.serial_number,
      a.data_source,
      coalesce(a.extra_metadata, '{}'::jsonb) as extra_metadata,
  ...
  filtered_rows as (
    ...
    where v_search is null
       ...
       or rr.extra_metadata #>> '{work_order,hull_number}' ilike '%' || v_search || '%'
       or rr.extra_metadata #>> '{work_order,order_number}' ilike '%' || v_search || '%'
       or rr.extra_metadata #>> '{build_code}' ilike '%' || v_search || '%'
       ...
       or (
         ...
         and regexp_replace(
           lower(concat_ws(
             ...
             rr.model,
             rr.serial_number,
             rr.extra_metadata #>> '{work_order,hull_number}',
             rr.extra_metadata #>> '{work_order,order_number}',
             rr.extra_metadata #>> '{build_code}',
    ...
    coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'template', jsonb_strip_nulls(jsonb_build_object(
          'template_key', p.extra_metadata ->> 'catalog_template_key',
          'manufacturer', p.make,
          'model', p.model,
          'model_year', p.year
        )),
        'exact_build', jsonb_strip_nulls(jsonb_build_object(
          'template_key', p.extra_metadata ->> 'catalog_template_key',
          'build_key', p.extra_metadata ->> 'exact_build_key',
          'build_code', p.extra_metadata ->> 'build_code',
          'hull_number', p.extra_metadata #>> '{work_order,hull_number}',
          'hin', p.serial_number,
          'order_number', p.extra_metadata #>> '{work_order,order_number}',
          'source_type', p.extra_metadata #>> '{provenance,source_type}',
          'source_document', p.extra_metadata #>> '{provenance,source_document}'
        )),
        'owner_state', case
          when (p.extra_metadata -> 'operating_states') is not null and jsonb_array_length(p.extra_metadata -> 'operating_states') > 0 then trim(both '"' from (p.extra_metadata -> 'operating_states' -> 0)::text)
        ...
        'verification', jsonb_build_object(
          ...
          'status', case when coalesce((p.extra_metadata ->> 'factory_confirmed')::boolean, false) then 'factory_confirmed' else null end
        ),
