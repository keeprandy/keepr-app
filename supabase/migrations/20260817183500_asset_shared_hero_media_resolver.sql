...
declare
  ...
  v_relationship_hero_placement_id uuid;
  v_media record;
begin
  ...
  if p_organization_id is not null then
    select case
        when coalesce(
          ar.metadata #>> '{presentation,hero_placement_id}',
          ar.metadata #>> '{presentation,heroPlacementId}',
          ar.metadata ->> 'hero_placement_id'
      ...
      into v_relationship_hero_placement_id
    from public.asset_relationships ar
    where ar.asset_id = p_asset_id
      and ar.organization_id = p_organization_id
      and ar.status = 'active'
  ...
  with candidates as (
    select
      ...
      att.kind,
      case
        when ap.id = v_relationship_hero_placement_id then 300
        when ap.id = v_asset.hero_placement_id then 200
        when coalesce(ap.role, '') in ('primary', 'hero') then 120
        when ap.is_showcase = true or coalesce(ap.role, '') = 'showcase' then 100
        else 0
      end as rank
    ...
    where ap.target_type = 'asset'
      ...
      and (
        ap.id = v_relationship_hero_placement_id
        or ap.id = v_asset.hero_placement_id
