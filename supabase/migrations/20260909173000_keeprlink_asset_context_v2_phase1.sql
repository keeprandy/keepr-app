    ...
    'systems', coalesce((
      select jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
        ...
        'facts', jsonb_strip_nulls(jsonb_build_object(
          'manufacturer', coalesce(s.metadata ->> 'manufacturer', s.ai_metadata ->> 'manufacturer', s.template_manufacturer),
          'model', coalesce(s.metadata ->> 'model', s.ai_metadata ->> 'model'),
          'serial_number', case when p_authorized then nullif(s.metadata ->> 'serial_number', '') else null end,
          'location', case when p_authorized then coalesce(s.metadata ->> 'location', s.metadata ->> 'position') else null end,
          'mode', s.mode,
