...
import {
  cancelScheduledReminderNotificationForReminder,
  scheduleOrReplaceReminderNotification,
...
export async function scheduleReminderPushNotification({
  ...
  try {
    return await scheduleOrReplaceReminderNotification({
      reminderId,
      title,
      body,
      dueAtISO,
    ...
    return { status: "failed", reason: error?.message || "schedule_failed" };
  }
}

export async function cancelReminderPushNotification(reminderId) {
  try {
    return await cancelScheduledReminderNotificationForReminder(reminderId);
  } catch (error) {
    console.log("Reminder push cancellation skipped:", error);
    return { status: "failed", reason: error?.message || "cancel_failed" };
  }
}
