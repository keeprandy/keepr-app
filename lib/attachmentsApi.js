...
function attachmentAttribution(attachment, contributorMetadata) {
  if (attachment?.source_context?.provenance === "model_template") {
    return attachment.source_context.provenance_label || "Model resource";
  }
...
export async function listAttachmentsForAsset(assetId, options = {}) {
  ...
  const includeInheritedTemplateAttachments = includeInheritedModelAttachments || includeInheritedModelMedia;
  ...
  if (ids.length === 0) {
    return includeInheritedTemplateAttachments
      ? await listInheritedTemplateAttachmentsForAsset(assetId, {
          mediaOnly: includeInheritedModelMedia && !includeInheritedModelAttachments,
        })
      : [];
  }
  ...
  if (!includeInheritedTemplateAttachments) return directRows;

  const inheritedRows = await listInheritedTemplateAttachmentsForAsset(assetId, {
    mediaOnly: includeInheritedModelMedia && !includeInheritedModelAttachments,
  });
...
function canonicalSystemTemplateIds(row) {
  const meta = metadataObject(row);
  const sourceContext = metadataObject(row?.source_context);
  const aiMetadata = metadataObject(row?.ai_metadata);
  const projection = metadataObject(meta.projection);
  const systemTemplate = metadataObject(meta.system_template);
  return compactUnique([
    row?.system_template_id,
    meta.system_template_id,
    meta.canonical_system_template_id,
    meta.system_template_ref_id,
    systemTemplate.id,
    projection.system_template_id,
    sourceContext.system_template_id,
    aiMetadata.system_template_id,
  ]).filter(isUuid);
}
...
export async function listInheritedTemplateResourcesForSystem(assetId, systemRow) {
  ...
  const { data: items, error: itemsErr } = await supabase
    ...
    .select("id,template_id,label,item_type,canonical_key,metadata,expected_value,system_template_id")
  ...
  const directSystemTemplateIds = canonicalSystemTemplateIds(systemRow);
  const itemSystemTemplateIds = matchedItems.flatMap(canonicalSystemTemplateIds);
  const systemTemplateIds = compactUnique([...directSystemTemplateIds, ...itemSystemTemplateIds]).filter(isUuid);

  let inheritedSystemTemplateResources = [];
  if (systemTemplateIds.length) {
    const { data: systemTemplates } = await supabase
      .from("system_templates")
      .select("id,canonical_key,name,manufacturer,system_category,authority_state,metadata")
      .in("id", systemTemplateIds);

    const systemTemplateById = new Map((systemTemplates || []).map((template) => [template.id, template]));
    const { data: systemTemplatePlacements, error: systemTemplatePlacementsErr } = await supabase
      .from("attachment_placements")
      .select(
        `
        *,
      ...
      .eq("target_type", "system_template")
      .in("target_id", systemTemplateIds)
      .order("created_at", { ascending: false });

    if (systemTemplatePlacementsErr) throw systemTemplatePlacementsErr;

    inheritedSystemTemplateResources = (systemTemplatePlacements || [])
      .filter((placement) => {
        const attachment = placement?.attachments || {};
        return attachment?.id && !attachment.deleted_at && !isMediaAttachment(attachment);
      })
      .map((placement) =>
        normalizeInheritedSystemTemplateResource(
          placement,
          systemTemplateById.get(placement.target_id),
          matchedItems.find((item) => canonicalSystemTemplateIds(item).includes(placement.target_id))
        )
      )
      .filter(Boolean);
  }

  const { data: attachmentPlacements, error: attachmentPlacementsErr } = await supabase
    .from("attachment_placements")
    .select(
      `
      *,
    ...
    .order("created_at", { ascending: false });
  if (attachmentPlacementsErr) throw attachmentPlacementsErr;

  const inheritedAttachmentResources = (attachmentPlacements || [])
    .filter((placement) => {
      const attachment = placement?.attachments || {};
      if (!attachment?.id || attachment.deleted_at) return false;
      if (isMediaAttachment(attachment)) return false;
      const linkedIds = attachmentLinkedTemplateItemIds(attachment);
      return linkedIds.some((id) => matchedItemById.has(id));
    })
    .map((placement) => {
      const attachment = placement?.attachments || {};
      const linkedIds = attachmentLinkedTemplateItemIds(attachment);
      ...
      return normalizeInheritedSystemAttachmentResource(
        placement,
  ...
  const inheritedLegacyResources = (resources || [])
  ...
  const byKey = new Map();
  [...inheritedSystemTemplateResources, ...inheritedAttachmentResources, ...inheritedLegacyResources].forEach((resource) => {
    const key = resource.attachment_id || resource.resource_id || resource.id;
...
function isMediaAttachment(attachment = {}) {
  ...
  ].filter(Boolean).join(" ").toLowerCase().split("?")[0].split("#")[0];
  return attachment.kind === "photo" || mime.startsWith("image/") || /\.(jpe?g|png|webp|gif|heic|heif|avif|svg)$/.test(name);
}

function attachmentLinkedTemplateItemIds(attachment = {}) {
  const sourceContext = metadataObject(attachment.source_context);
  const aiMetadata = metadataObject(attachment.ai_metadata);
  return [
...
function normalizeInheritedSystemAttachmentResource(placement, template, item) {
  const attachment = placement?.attachments || {};
  const sourceContext = metadataObject(attachment.source_context);
  return {
    id: attachment.id || placement.attachment_id,
    attachment_id: attachment.id || placement.attachment_id,
    placement_id: placement.id || null,
    title: attachment.title || placement.label || attachment.file_name || "Reusable resource",
    resource_type: placement.role || attachment.ai_metadata?.role || attachment.kind || "resource",
    kind: attachment.kind || null,
    url: resourceUrl(attachment),
    source_url: attachment.url || sourceContext.source_url || null,
    source_name: sourceContext.source_name || null,
    template_id: template?.id || placement.target_id || null,
    template_key: template?.template_key || sourceContext.template_key || null,
    template_label: modelTemplateLabel(template),
    template_item_id: item?.id || sourceContext.template_item_id || null,
    template_item_label: item?.label || sourceContext.template_item_label || null,
    provenance: "model_template_item",
    provenance_label: sourceContext.provenance_label || "Inherited model knowledge",
    provenance_detail:
      sourceContext.provenance_detail ||
      "Reusable model/system resource inherited through this asset's model binding; not exact-hull evidence.",
    not_exact_hull_evidence: true,
    source_context: sourceContext,
    ai_metadata: attachment.ai_metadata || null,
  };
...
function normalizeInheritedSystemTemplateResource(placement, systemTemplate, item) {
  const attachment = placement?.attachments || {};
  const sourceContext = metadataObject(attachment.source_context);
  const label = systemTemplate?.name || sourceContext.system_template_name || item?.label || "System Template";
  return {
    id: attachment.id || placement.attachment_id,
    attachment_id: attachment.id || placement.attachment_id,
    placement_id: placement.id || null,
    ...
    url: resourceUrl(attachment),
    source_url: attachment.url || sourceContext.source_url || null,
    source_name: sourceContext.source_name || systemTemplate?.manufacturer || null,
    system_template_id: systemTemplate?.id || placement.target_id || null,
    system_template_key: systemTemplate?.canonical_key || sourceContext.system_template_key || null,
    system_template_label: label,
    template_item_id: item?.id || sourceContext.template_item_id || null,
    template_item_label: item?.label || sourceContext.template_item_label || null,
    provenance: "system_template",
    provenance_label: sourceContext.provenance_label || `${label} reusable system knowledge`,
    provenance_detail:
      sourceContext.provenance_detail ||
      "Inherited from the canonical System Template by reference; not exact-system evidence.",
    not_exact_hull_evidence: true,
    not_exact_system_evidence: true,
    source_context: {
      ...sourceContext,
      provenance: "system_template",
      system_template_id: systemTemplate?.id || placement.target_id || null,
      system_template_key: systemTemplate?.canonical_key || sourceContext.system_template_key || null,
      system_template_name: label,
      not_exact_system_evidence: true,
    },
    ai_metadata: attachment.ai_metadata || null,
  };
...
function normalizeTemplateAttachmentPlacement(templatePlacement, template, assetPlacement) {
  const row = normalizeTemplateMediaPlacement(templatePlacement, template, assetPlacement);
  const attachment = templatePlacement?.attachments || {};
  const sourceContext = attachment.source_context && typeof attachment.source_context === "object"
    ? attachment.source_context
    : {};
  const role = templatePlacement.role || attachment.ai_metadata?.role || attachment.kind || "resource";
  const roleLabel = labelizeRole(role);
  const provenanceLabel =
    sourceContext.provenance_label ||
    `${modelTemplateLabel(template)} model resource`;

  return {
    ...row,
    kind: attachment.kind || row.kind || "file",
    role,
    title: attachment.title || templatePlacement.label || attachment.file_name || roleLabel,
    label: assetPlacement?.label || templatePlacement.label || attachment.title || roleLabel,
    is_inherited_model_attachment: true,
    is_inherited_model_media: isMediaAttachment(attachment),
    is_exact_asset_media: false,
    source_context: {
      ...row.source_context,
      ...sourceContext,
      ...
      provenance_detail:
        sourceContext.provenance_detail ||
        `Inherited ${roleLabel.toLowerCase()} from the bound model template; not exact-hull evidence.`,
      template_id: template?.id || templatePlacement.target_id || null,
      template_key: template?.template_key || sourceContext.template_key || null,
      not_exact_hull_evidence: true,
    },
    ai_metadata: attachment.ai_metadata || null,
    attribution: provenanceLabel,
    provenance_label: provenanceLabel,
    provenance_detail:
      sourceContext.provenance_detail ||
      `Inherited ${roleLabel.toLowerCase()} from the bound model template; not exact-hull evidence.`,
...
function labelizeRole(value) {
  const raw = String(value || "resource").trim();
...
export async function listModelTemplateMediaForTemplates(templates = []) {
  ...
  for (const placement of placements || []) {
    ...
    byTemplateId[placement.target_id].media.push(row);
    if (row.is_hero) {
      byTemplateId[placement.target_id].hero = row;
    }
  }

  const hydrateHero = async (row) => {
    if (!row) return null;
    ...
    return signedUrl ? { ...row, signed_url: signedUrl, url: row.url || signedUrl } : row;
  };

  await Promise.all(Object.values(byTemplateId).map(async (entry) => {
    if (!entry.hero) {
      entry.hero =
    ...
    entry.hero = await hydrateHero(entry.hero);
  }));

  return byTemplateId;
}

async function listInheritedTemplateAttachmentsForAsset(assetId, options = {}) {
  ...
  const templateAttachmentRows = (templatePlacements || [])
    .filter((placement) => {
      if (!mediaOnly) return true;
      return isMediaAttachment(placement.attachments || {});
    })
    .map((placement) => {
      const normalizer = mediaOnly
        ? normalizeTemplateMediaPlacement
        : normalizeTemplateAttachmentPlacement;
      return normalizer(
        placement,
        templateById.get(placement.target_id),
  ...
  const seen = new Set();
  return [...templateAttachmentRows, ...legacyResourceRows].filter((row) => {
...
async function listInheritedTemplateMediaForAsset(assetId) {
  return listInheritedTemplateAttachmentsForAsset(assetId, { mediaOnly: true });
}
