...
export function getMatchingOpenThreads(threads = [], { assetId, systemId, recipientId } = {}) {
  if (!assetId) return [];
  return threads
    .filter((thread) => {
...
export async function resolveMessageIdentities({
  ...
  const priorProfiles = missingProfileIds.length
    ? await fetchByIds("profiles", missingProfileIds, "id, display_name, full_name, email, avatar_url, photo_url")
    : [];
  const profileLookup = {
    ...(profilesById || {}),
  ...
  if (currentUserId) {
    const { data: pros } = await supabase
      .from("keepr_pros")
      .select("id, name, category, email, phone, website, location, is_favorite")
    ...
    (pros || []).forEach((pro) => {
      ...
      addIdentity(byKey, {
        ...
        display_name: pro.name || "KeeprPro",
        email: pro.email || null,
        user_id: null,
        keepr_pro_id: pro.id,
        organization_name: pro.name || null,
        relationship_label: relationship,
        availability_state: "not_message_enabled",
        avatar_url: null,
        is_selectable: false,
        disabled_reason: "Not yet participating in Keepr Messages.",
        context_relevance: connected ? "Connected KeeprPro" : previousThread?.subject || "Saved KeeprPro contact",
...
export async function loadEligibleRecipientsForAsset(assetId, currentUserId, { hubId, force = false } = {}) {
  ...
  const profiles = await fetchByIds(
    "profiles",
    Array.from(ids),
    "id, display_name, full_name, email, avatar_url, photo_url"
  );
...
export async function loadThreadMessages(threadId, {
  ...
  const messages = dedupeMessages(descending);
  const result = {
    currentUserId,
    messages,
