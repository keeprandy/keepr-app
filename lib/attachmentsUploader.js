...
import { normalizeContributionMetadata } from "./provenance";
...
const profileLabelCache = new Map();
...
function normalizeSourceContext(value) {
  ...
  return { legacy_source_context: String(value) };
}

async function getContributorLabel(userId) {
  if (!userId) return null;
  if (profileLabelCache.has(userId)) return profileLabelCache.get(userId);
  try {
    ...
    const label = data?.display_name || data?.full_name || data?.email || null;
    profileLabelCache.set(userId, label);
    return label;
  } catch {
    profileLabelCache.set(userId, null);
    return null;
  }
}

async function buildSourceContext({ sourceContext, userId, assetRelationshipId, stewardshipId }) {
  const base = normalizeSourceContext(sourceContext);
  const orgId =
    base.contributed_by_org_id ||
    base.organization_id ||
    base.provider_organization_id ||
    base.org_id ||
    null;
  const orgLabel =
    base.contributed_by_org_label ||
    base.provider_name ||
    base.organization_name ||
    null;
  const userLabel =
    base.contributed_by_user_label ||
    base.completed_by_label ||
    (await getContributorLabel(userId));

  return normalizeContributionMetadata({
    metadata: {
      ...base,
      contributed_by_user_label: userLabel || undefined,
      contributed_by_org_label: orgLabel || undefined,
    },
    userId,
    orgId,
    assetRelationshipId: assetRelationshipId || base.asset_relationship_id || null,
    stewardshipId: stewardshipId || base.stewardship_id || null,
    contributionContext: base.contribution_context || base.origin || base.screen || "attachment_upload",
    authorityState: base.authority_state || "official",
  });
}
...
async function maybeSetInitialAssetHero({ assetId, placementRows, attachment }) {
  ...
  const assetPlacement = (placementRows || []).find(
    (p) => {
      if (p?.target_type !== "asset" || p?.target_id !== assetId) return false;
      const role = String(p?.role || "").toLowerCase();
      return role === "hero" || role === "primary" || role === "showcase" || p?.is_showcase;
    }
  );
  ...
  if (!updateError && updatedAsset?.id) return;

  const { error: rpcError } = await supabase.rpc("set_asset_hero_placement", {
    p_asset_id: assetId,
    p_placement_id: assetPlacement.id,
  });

  if (rpcError) {
    throw new Error(`Could not set asset hero: ${rpcError.message}`);
  }
}
...
export async function uploadAttachmentFromUri({
  ...
  const finalSourceContext = await buildSourceContext({
    sourceContext,
    userId,
  });
  ...
  const attachment = await insertAttachmentRow({
    ...
    notes: notes ?? null,
    source_context: finalSourceContext,
  });
...
if (kind === "photo" && assetId && setAsAssetHero) {
...
export async function createLinkAttachment({
  ...
  const finalSourceContext = await buildSourceContext({
    sourceContext: sourceContext || inferred.source_context,
    userId,
  });
...
export async function createTextAttachment({
  ...
  const attachment = await insertAttachmentRow({
    ...
    title,
    notes: body,
    source_context: await buildSourceContext({
      sourceContext: "shared_text",
      userId,
    }),
  });
