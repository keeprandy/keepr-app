...
import { normalizeImageAssetForUpload } from "./imagePickerAssets";
...
export async function pickAndNormalizeImageFromLibrary() {
  ...
  const normalized = await normalizeImageAssetForUpload(result.assets[0], `invoice-${Date.now()}.jpg`);
  return normalized?.uri || null;
}
