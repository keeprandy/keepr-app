...
function renderEvidence(projection) {
  ...
  const placements = asArray(projection.evidence_placements);
  return fieldList({
    "Attachment count": evidence.attachment_count || evidence.count || placements.length,
...
export function renderKeeprLinkAiDocument(projection, { canonicalJsonUrl = "", canonicalPageUrl = "" } = {}) {
  ...
  const context = Object.keys(asObject(safeProjection.projection)).length
    ? asObject(safeProjection.projection)
    : safeProjection;
  const object = asObject(context.object);
  const contextIdentity = asObject(context.identity);
  const contextSummary = asObject(context.context_summary);
  const manifestVersion = safeString(safeProjection.manifest_version || context.manifest_version) || "unknown";
  const generatedAt = safeString(safeProjection.generated_at) || new Date().toISOString();
  const kac = safeString(object.kac_id) || safeString(context.kac_id) || safeString(context.address).replace(/^\/k\//, "");
  const title = safeString(contextIdentity.name) || safeString(object.name) || safeString(context.title) || kac || "KeeprLINK Context";
  const identity = {
    "KAC": kac,
    "Name": title,
    "Type": object.type || safeProjection.object_type,
    "Asset type": contextIdentity.asset_type,
    "Year": contextIdentity.year,
    "Make": contextIdentity.make,
    "Model": contextIdentity.model,
...
<main>
  ...
  <section id="systems"><h2>Systems</h2>${renderSystems(context)}</section>
  <section id="resources"><h2>Resources</h2>${renderResources(context)}</section>
  <section id="operational-history"><h2>Operational History</h2>${renderHistory(context)}</section>
  <section id="relationships"><h2>Relationships</h2>${renderRelationships(context)}</section>
  <section id="evidence"><h2>Evidence</h2>${renderEvidence(context)}</section>
  <section id="gaps"><h2>Gaps</h2>${renderGaps(context)}</section>
