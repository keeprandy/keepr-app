...
export function invalidateAssetHeroCache(assetId = null) {
  if (!assetId) {
    heroUriCache.clear();
    heroUriByAssetCache.clear();
    return;
  }

  heroUriByAssetCache.delete(assetId);
  Array.from(heroUriCache.keys()).forEach((key) => {
    if (String(key).startsWith(`${assetId}::`)) heroUriCache.delete(key);
  });
}
...
function heroCandidatePlacementFilter(row) {
  ...
  return role === "hero" || role === "showcase" || role === "photo" || Boolean(row?.is_showcase);
}

function placementRank(row) {
  const role = String(row?.role || "").toLowerCase();
  if (role === "primary" && row?.is_showcase) return 100;
...
async function resolvePlacementHeroUri(placementId, transform, expiresIn, { requireHeroEligible = false } = {}) {
  if (!placementId) return null;

  const { data, error } = await supabase
    .from("attachment_placements")
    .select(
      "id,role,is_showcase,attachment:attachments(id,bucket,storage_path,url,mime_type,kind,deleted_at,title,file_name)"
    )
    .eq("id", placementId)
    .maybeSingle();
  ...
  if (requireHeroEligible && !heroCandidatePlacementFilter(data)) return null;
  if (!imageAttachmentFilter(data)) return null;
