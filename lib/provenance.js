...
export function getContributionMetadata(row = {}) {
  ...
  return {
    ...asObject(safeRow.source_context),
    ...asObject(safeRow.extra_metadata),
    ...asObject(safeRow.ai_metadata?.source_context),
  };
}
...
export function formatContributionAttribution(row = {}, options = {}) {
  ...
  const metadata = {
    ...getContributionMetadata(safeRow),
    ...asObject(safeOptions.metadata),
  };
  const meta = {
    ...metadata,
    contributed_by_user_id: firstPresent(safeRow.contributed_by_user_id, metadata.contributed_by_user_id),
    contributed_by_org_id: firstPresent(safeRow.contributed_by_org_id, metadata.contributed_by_org_id),
    contributed_by_user_label: firstPresent(safeRow.contributed_by_user_label, metadata.contributed_by_user_label),
    contributed_by_org_label: firstPresent(safeRow.contributed_by_org_label, metadata.contributed_by_org_label),
    contribution_context: firstPresent(safeRow.contribution_context, metadata.contribution_context),
    authority_state: firstPresent(safeRow.authority_state, metadata.authority_state),
  };

  const context = String(
    firstPresent(meta.contribution_context, meta.source_type, safeRow.source_type, safeRow.source_context) || ""
  ).toLowerCase();
