...
function isHeicImage({ contentType, fileName }) {
  const ct = safeLower(contentType);
  const name = safeLower(fileName);
  return ct.includes("heic") || ct.includes("heif") || /\.(heic|heif)$/.test(name);
}

async function normalizeHeicImage({ localUri, contentType, fileName }) {
  if (Platform.OS === "web" || !localUri || !isHeicImage({ contentType, fileName })) {
    return { localUri, contentType, fileName };
  }
...
export async function uploadLocalFileToSupabase({
  ...
  try {
    ...
    const heicNormalized = await normalizeHeicImage({
      localUri,
      contentType: webFile?.type || contentType,
      fileName: fileName || webFile?.name || null,
    });

    localUri = heicNormalized.localUri;

    const inferredName = heicNormalized.fileName || webFile?.name || null;
    const inferredType =
      webFile?.type || heicNormalized.contentType || "application/octet-stream";
