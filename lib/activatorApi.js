...
export async function listSystemTemplates({ query = "", limit = 25, organizationId = null, scope = "all" } = {}) {
...
export async function listSupplierNetwork({ organizationId, query = "", limit = 50 } = {}) {
  if (!organizationId) return { suppliers: [], counts: {} };

  const { data, error } = await supabase.rpc("list_organization_supplier_network", {
    p_organization_id: organizationId,
    p_query: query,
    p_limit: limit,
  });
  ...
  if (error) throw normalizeActivatorError(error);
  return data || { suppliers: [], counts: {} };
}

function systemTemplatePayload(input = {}) {
  ...
  return {
    ...
    authority_state: input.authorityState || "draft",
    metadata,
  };
}

export async function getSystemTemplate(systemTemplateId) {
  if (!systemTemplateId) return null;

  const { data, error } = await supabase
    .from("system_templates")
    .select("id,canonical_key,name,manufacturer,supplier_org_id,owner_org_id,system_category,description,authority_state,metadata,created_at,updated_at")
...
export async function upsertSystemTemplate(input = {}) {
  const payload = systemTemplatePayload(input);
  if (!payload.name || !payload.canonical_key) {
    throw new Error("System Template name and canonical key are required.");
  }

  const query = input.id
    ? supabase.from("system_templates").update(payload).eq("id", input.id).select("*").single()
    : supabase.from("system_templates").insert(payload).select("*").single();
...
export async function retireSystemTemplate(systemTemplateId, reason = "retired_from_system_library") {
  if (!systemTemplateId) {
    throw new Error("System Template id is required.");
  }

  const current = await getSystemTemplate(systemTemplateId);
  if (!current) {
...
export async function promoteModelItemToSystemTemplate({ templateItemId, payload = {} } = {}) {
  const { data, error } = await supabase.rpc("promote_model_item_to_system_template", {
    p_template_item_id: templateItemId,
    p_payload: payload || {},
  });
