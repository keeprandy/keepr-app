...
function systemTemplateIdForItem(item) {
  const metadata = item?.metadata && typeof item.metadata === "object" ? item.metadata : {};
  const projection = cleanProjection(metadata.projection);
  const systemTemplate = metadata.system_template && typeof metadata.system_template === "object"
    ? metadata.system_template
    : {};
  return (
    item?.system_template_id ||
    metadata.system_template_id ||
    metadata.canonical_system_template_id ||
    systemTemplate.id ||
...
export function normalizeTemplateItemProjection(item, group = null) {
  ...
  const systemTemplateId = systemTemplateIdForItem(item);
  ...
  if (explicitKind) {
    return {
      ...
      system_template_id: explicit.system_template_id || systemTemplateId,
    };
  }
...
function buildEligibleConfiguration(configurationGroups, resources = []) {
  return configurationGroups.flatMap(({ group, children }) => children.map((item) => {
    ...
    return {
      ...
      systemTemplateId: projection.system_template_id || systemTemplateIdForItem(item),
      system_template_id: projection.system_template_id || systemTemplateIdForItem(item),
      projection,
