...
import { buildHubParticipationMetadata } from "./hubConfig";
...
export async function fetchHubStoryLinks(hubId, { includeReviewStatuses = false } = {}) {
  if (!hubId) return [];
  ...
  return (data || [])
    .filter((row) => includeReviewStatuses || (row.status || "approved") === "approved")
    .map((row) => ({
      ...row,
      asset: row.asset || null,
      ownerProfile: row.owner_profile || null,
...
export async function addStoryToHub({
  hubId,
  assetId,
  userId,
  status = "approved",
  ...
  const cleanStatus = ["approved", "pending", "declined"].includes(status)
    ? status
    : "approved";
  const extraMetadata = {
    ...(hub ? buildHubParticipationMetadata(hub) : {}),
    ...metadata,
  };
  const insertPayload = {
    hub_id: hubId,
  ...
  if (Object.keys(extraMetadata).some((key) => extraMetadata[key] != null)) {
    insertPayload.extra_metadata = extraMetadata;
  }
  ...
  if (
    error &&
    insertPayload.extra_metadata &&
    /extra_metadata|schema cache|column/i.test(error.message || "")
  ) {
    const { extra_metadata, ...fallbackPayload } = insertPayload;
...
export async function inviteHubMember({
  ...
  const cleanNote = String(personalNote || "").trim();
...
if (status === "invited") {
  const err = new Error(`This email already has a pending invite as ${roleLabel}.`);
  err.code = "INVITE_ALREADY_PENDING";
  err.existingMembership = existingMembership;
  throw err;
}
...
export async function approveHubStoryLink(linkId) {
  return updateHubStoryLink(linkId, {
    status: "approved",
  });
}

export async function declineHubStoryLink(linkId) {
  return updateHubStoryLink(linkId, {
    status: "declined",
  });
}
...
export async function acceptHubInviteByToken({ inviteToken, userId }) {
  ...
  const { data, error } = await supabase.functions.invoke("accept-hub-invite", {
    body: { inviteToken },
  });
