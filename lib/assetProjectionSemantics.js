...
export function assetProjectionSemantics({
  ...
  const relationshipStatus = normalize(relationship.status || asset.relationship_status);
  const claimState = normalize(relationship.claim_state || asset.claim_state);
  const workspaceType = normalize(workspace.workspace_type || workspace.type);
  const ownerPresent = hasActualOwner(asset);

  const isOwner =
    (relationshipType === "owner" && (relationshipStatus === "active" || claimState === "accepted")) ||
    accessScope === "owner_full" ||
    workspaceType === "keepr";
  const isPendingOwner =
    relationshipType === "owner" &&
    !isOwner &&
    ["pending", "invited"].includes(relationshipStatus || claimState);
  const isInventory =
  ...
  return {
    projection: isPendingOwner ? "pending_owner" : projection,
    relationshipType,
    accessScope,
    relationshipStatus,
    claimState,
    ownerPresent,
    modeLabel: isPendingOwner ? "Pending Handoff" : "Workspace",
    workspaceRoleLabel: relationshipLabel || "Workspace",
    workspaceRoleIcon: "briefcase-outline",
    relationshipLabel,
    relationshipTitle: isPendingOwner ? `${assetName} · Pending owner handoff` : `${assetName} · Workspace projection`,
    headerEyebrow: isPendingOwner ? "Pending owner handoff" : "Workspace",
    heroContext: isPendingOwner ? "Pending handoff" : "Workspace projection",
    heroFallbackMeta: "Photo will appear here when shared",
    summaryLabel: isPendingOwner ? "Pending" : "Workspace",
    summaryHint: isPendingOwner
      ? "Accept the handoff before this KAC enters your ownership view."
      : "This object is visible because of a workspace relationship.",
