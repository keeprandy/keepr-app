...
import { invalidateAssetHeroCache } from "./assetHeroResolver";
...
function isRpcSignatureCacheMiss(error) {
...
export async function setKacHero({
  assetId,
  placementId,
  actorContext = {},
} = {}) {
  ...
  const payload = {
    ...
    p_actor_context: actorContext || {},
  };

  let { data, error } = await supabase.rpc("set_asset_hero_placement", payload);
  if (error && isRpcSignatureCacheMiss(error)) {
    ({ data, error } = await supabase.rpc("set_asset_hero_placement", {
      p_asset_id: assetId,
      p_placement_id: placementId,
    }));
  }
  ...
  if (error) throw error;
  invalidateAssetHeroCache(assetId);
  return data || null;
}
...
export async function clearKacHero({
  assetId,
  actorContext = {},
} = {}) {
  if (!assetId) throw new Error("Missing asset id.");

  let { data, error } = await supabase.rpc("clear_asset_hero_placement", {
    p_asset_id: assetId,
    p_actor_context: actorContext || {},
  });
  if (error && isRpcSignatureCacheMiss(error)) {
    ({ data, error } = await supabase.rpc("clear_asset_hero_placement", {
      p_asset_id: assetId,
    }));
  }
...
export async function searchKeeprSpaceOrganizations(query = "", filters = {}) {
  const { data, error } = await supabase.rpc("search_keeprspace_organizations", {
    p_query: query?.trim?.() || "",
