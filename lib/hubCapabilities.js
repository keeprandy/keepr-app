import { getHubParticipationConfig } from "./hubConfig";
...
export function getHubUserCapabilities({
  ...
  const primaryAssetType = String(
    participationConfig.primaryAssetType ||
  ...
  const addAssetAction =
    ownerControlled
      ...
      ? "invite_required"
      : participationConfig.submissionStatus === "pending"
      ? "submit_pending"
      : moderated
      ? "request"
      : "add";
  ...
  const canOpenQuickActivation =
    isPublicHub &&
    participationConfig.canQuickActivate &&
    publicParticipation &&
    primaryAssetType === "vehicle";

  const addAssetLabel = participationConfig.ctaLabel;

  return {
    hubType,
    primaryAssetType,
    presetKey: participationConfig.presetKey,
    submissionStatus: participationConfig.submissionStatus,
    visibility,
    participation,
    role,
