...
function porscheManifest(overrides = {}) {
  return {
    ...
    associations: [
      ...
      assoc({ association_id: "work_event:service_record:usage-1", object_id: "svc-usage-1", object_type: "work_event", source_table: "service_records", relationship_type: "service_record", event_role: "usage", proof_state: "claimed", safe_metadata: { title: "Mileage update" }, created_at: "2026-03-01T00:00:00Z" }),
...
test("well-documented low-mileage Porsche may return no_action_required with empty plan", async () => {
  const brief = makeBrief(porscheManifest({
    ...
    associations: [
      ...
      assoc({
        association_id: "work_event:service_record:low-mileage",
        object_id: "svc-low-mileage",
...
test("low marine hours are respected and do not create unsupported service work", async () => {
  ...
  const manifest = porscheManifest({
    ...
    associations: [
      ...
      assoc({ association_id: "work_event:service_record:low-hours", object_id: "svc-low-hours", object_type: "work_event", source_table: "service_records", relationship_type: "service_record", event_role: "usage", proof_state: "verified", safe_metadata: { title: "Low engine hours", engine_hours: 12, usage: "low" }, created_at: "2026-04-01T00:00:00Z" }),
    ],
    knowledge_gaps: [],
  });
  const brief = makeBrief(manifest, "maintenance_planning");
